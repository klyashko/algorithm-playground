...
class SegmentSumTreeSpec extends Specification {
	...
	def "segmentSumTree"() {
		...
		given:
			...
			def sums = sum(nums)
			println("Sums: ${sums}")
			def tree = new SegmentSumTree(nums as int[])
	...
	private static List<List<Integer>> produce(SegmentSumTree tree, int[] sums) {
		def len = sums.size() - 1
		return (0..10).collect {
