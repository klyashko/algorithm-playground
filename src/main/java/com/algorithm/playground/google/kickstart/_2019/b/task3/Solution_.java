...
public class Solution_ {
	...
	static class DiverseSubarray {
		...
		void insert(int level, int index, int value) {
...
//			int depth = 18;
			...
			for (int scene : occurrences.keySet()) {
				for (int j = 1; j <= s && j - 1 < occurrences.get(scene).size(); j++) {
					insert(depth, occurrences.get(scene).get(j - 1), 1);
				}
				if (occurrences.get(scene).size() >= s + 1) {
					insert(depth, occurrences.get(scene).get(s), -s);
				}
			}
			...
			for (int start = 0; start < N; start++) {
				if (max_psum[0][0] > answer) {
					answer = max_psum[0][0];
				}

				int scene = M[start];
				int off = offsets.get(scene);
				insert(depth, start, 0);

				if (occurrences.get(scene).size() >= s + 1 + off) {
					insert(depth, occurrences.get(scene).get(s + off), 1);
				}
				if (occurrences.get(scene).size() >= s + 2 + off) {
					insert(depth, occurrences.get(scene).get(s + 1 + off), -s);
				}
